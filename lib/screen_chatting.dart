...
late String receiverId,receiverName,status;
...
class ChattingScreen extends StatefulWidget {
  ...
  const ChattingScreen(this.receiverId,this.receiverName, this.imageProviderC,this.status, {super.key});
  @override
...
//-------------SendButton
                      IconButton(
                        ...
                        onPressed: () async
                        {
                          ...
                          if (messageText.isNotEmpty) {
                            ...
                            await insertMessagesToFirebase(chattingId, message,
                                timestamp);
                          }
                        },
          ...
          InkWell(
            onTap: () {
              Navigator.push(context, MaterialPageRoute(
                  builder: (context) => UserProfile(receiverId, false)));
            },
            child: Column(
  ...
  StreamBuilder<QuerySnapshot<Object?>> getMessage() {
    return StreamBuilder(
        ...
        builder: (context, snapshot) {
          ...
          var firebaseData = snapshot.data!
              .docs; //----------firebase chatId message data
  ...
  StreamBuilder<List<Message>> getMessages() {
    return StreamBuilder<List<Message>>(
      stream: db.watchChatMessages(
          loggedUserId.toString(), receiverId.toString()),
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          return const Center(
